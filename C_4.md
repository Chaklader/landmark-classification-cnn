...
$$
\begin{align}
\text{Customer}_1 &: [Age=25, Income=\$40k, Purchases/Month=5] \\
\text{Customer}_2 &: [Age=55, Income=\$80k, Purchases/Month=2] \\
\text{Customer}_3 &: [Age=23, Income=\$35k, Purchases/Month=6] \\
\text{Customer}_4 &: [Age=52, Income=\$75k, Purchases/Month=2] \\
&\vdots
\end{align}
$$
...
$$
\begin{align}
\text{Student}_1 &: [Math=85, Physics=82, Chemistry=80, Biology=70, ...] \\
\text{Student}_2 &: [Math=92, Physics=90, Chemistry=88, Biology=75, ...] \\
&\vdots
\end{align}
$$
...
$$
\begin{align}
\text{Student}_1 &: [STEM_Score=0.85, LifeSci_Score=0.32] \\
\text{Student}_2 &: [STEM_Score=0.92, LifeSci_Score=0.41] \\
&\vdots
\end{align}
$$
...
$$
\begin{align}
\text{Customer}_A &: [Hours_Spent_Shopping=2, Items_Purchased=5] \\
\text{Customer}_B &: [Hours_Spent_Shopping=3, Items_Purchased=8] \\
\text{Customer}_C &: [Hours_Spent_Shopping=8, Items_Purchased=15] \\
\text{Customer}_D &: [Hours_Spent_Shopping=9, Items_Purchased=18] \\
\text{Customer}_E &: [Hours_Spent_Shopping=1, Items_Purchased=3] \\
\text{Customer}_F &: [Hours_Spent_Shopping=7, Items_Purchased=12] \\
\end{align}
$$
...
$$
\begin{align}
\text{Cluster}_1 &: [Hours_Spent_Shopping=4, Items_Purchased=7] \\
\text{Cluster}_2 &: [Hours_Spent_Shopping=6, Items_Purchased=14]
\end{align}
$$
...
$$
\begin{align}
\text{Cluster}_1 &: [(2+3+1)/3, (5+8+3)/3] = [2, 5.33] \\
\text{Cluster}_2 &: [(8+9+7)/3, (15+18+12)/3] = [8, 15]
\end{align}
$$
...
```
Traditional Approach: Raw Data → Single Unsupervised Method → One Type of Insight
Autoencoder Approach: Raw Data → Encoder-Decoder Architecture → Multiple Insights Simultaneously
```
...
```
Human: "This image is a cat"
Network: Learns Input → "cat" label
Requires: Thousands of images with human-provided labels
```

**Autoencoder Self-Supervised Learning:**

```
Network to itself: "Can I recreate this image perfectly after compressing it?"
Network: Learns Input → Compressed → Reconstructed Input
Requires: Just the raw images, no labels needed!
```
...
**Loss Function as Self-Evaluation:** $$\mathcal{L}(x, \hat{x}) = \mathcal{L}(x, g(f(x)))$$
...
&\text{where layer } k \text{ in the encoder corresponds positionally to layer } l \text{ in the decoder}
\end{align}
$$
