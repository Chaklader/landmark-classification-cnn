...
In color image convolution, our kernel expands from a simple k×k matrix to a k×k×n_channels volume, where n_channels
matches the number of channels in the input (typically 3 for RGB images). Each channel in the input has its own
corresponding filter weights in the kernel.
